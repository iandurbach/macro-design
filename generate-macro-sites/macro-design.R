...
#########
### User specifies everything here
#########

# path to shape file of study area (needed)
path_to_study_area <- "data/Occu15x15_PotSL_wstrata.shp"

# path to csv of centre of any already surveyed areas (NULL if none)
path_to_surveyed_pts <- NULL
#path_to_surveyed_pts <- "data/surveyed_points_example.csv"

# buffer (in meters) to place around any existing survey points
buffer <- 10000

# stratification options

# stratification variable
strat_var <- "RASTERVALU"

# breaks for categorising continuous stratification variable (scalar for same N cells in each stratum)
strat_breaks <- c(0, 0.5, 0.8, 1)

# proportion to sample from each strata (leave NULL for area-proportional sampling)
strat_proportions <- NULL

# random seed for Halton
initial_seed <- 1234
...
# basemap for plotting (other good basemaps "Esri.WorldImagery", "OpenStreetMap")
...
already_surveyed <- load_already_surveyed_areas(path_to_surveyed_pts, buffer = buffer, lat_varname = "lat", lng_varname = "lng")
...
paste0("Spatial balance is ", round(BalancedSampling::sb(p, X, s), 3))
