...
#nc_sf <- st_read("data/Occu15x15_PotSL_wstrata.shp")
...
server <- function(input, output, session) {
  ...
  # stratum proportions
  strat_proportions <- eventReactive(nc_sf(), {
  ...
  pts_sf_ll <- eventReactive(c(input$seed, input$dens_file, input$survarea_file), {
  ...
  observe({
    leafletProxy("map") %>%
      clearMarkers() %>%
      addPolygons(data = already_surveyed()$latlong, weight = 0, color = "black")
  })
